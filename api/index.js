...
export default async function handler(req, res) {
  try {
    ...
    // ← ここがポイント：/api と /api/index の両方を剥がして相対パス化
    const rel = path
      .replace(/^\/api(?:\/index)?\/?/, "")   // "/api/login" も "/api/index/login" も "login" に
      .replace(/\/+$/, "");                   // 末尾スラッシュ除去
      ...
      const sql = `
        ...
        ranked AS (
          SELECT *,
            ROW_NUMBER() OVER (
              PARTITION BY kind
              ORDER BY times ASC, COALESCE(last_at, 'epoch') ASC, created_at ASC
            ) AS rnk
          FROM appl
        )
